...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({
      ...
      region_1depth: result?.region_1depth_name ?? "",
      region_2depth: result?.region_2depth_name ?? "",
      region_3depth: result?.region_3depth_name ?? "",
    ...
    return Response.json({ error: "Kakao reverse geocode request failed." }, { status: 502 });
  }
}
