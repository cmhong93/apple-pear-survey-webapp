...
export async function appendMediaFiles(media: MediaArtifact[]) {
  return appendRows(
    ...
    media.map((item) => [
      ...
      item.geminiQaImageMeta?.mimeType ?? '',
      item.geminiQaImageMeta?.width ?? '',
      item.geminiQaImageMeta?.height ?? '',
      item.geminiQaImageMeta?.maxLongSide ?? '',
      item.geminiQaImageMeta?.jpegQuality ?? '',
      item.geminiQaImageMeta?.metadataRemoved ?? '',
      item.geminiQaImageMeta?.byteSize ?? '',
      item.extractedMyGps660Coordinate?.lat ?? '',
      item.extractedMyGps660Coordinate?.lng ?? '',
      item.extractedMyGps660Coordinate?.confidence ?? '',
      item.manualMyGps660Coordinate?.lat ?? '',
      item.manualMyGps660Coordinate?.lng ?? '',
      item.gpsCrossCheckStatus ?? '',
      item.gpsCrossCheckMessage ?? '',
    ]),
  )
}
